...
cover:
    zabbix游戏日志监控: "posts/blog/zabbix-mysql8.0/Typora_11_jiankongsystem1.svg" #图片路径例如：posts/tech/123/123.png
    caption: "" #图片底部描述
    alt: ""
    relative: false
---
...
![image-20220211220534064](Typoraimage-20220211220534064.png)
...
![Typoraimage-20220211221904380](Typoraimage-20220211221904380.png)

### 安装出现的问题

#### Minimum required size of PHP post is 16M (configuration option "post_max_size").

![image-20220601181428220](image-20220601181428220.png)
...
![image-20220601194844884](image-20220601194844884.png)
...
### 卸载 `zabbix`
 ...
 6. 查询出冗余文件并删除
    ...
    ![Typoraimage-20220216161855867](Typoraimage-20220216161855867.png)
...
![image-20220601201553650](image-20220601201553650.png)

![image-20220601201145104](image-20220601201145104.png)
...
![Typoraimage-20220211222333409](Typoraimage-20220211222333409.png)

<font color='red'>**在点击完new dashboard 按钮以后 按ctrl + s 保存一个自己定义的仪表盘**</font>

![Typoraimage-20220211222517727](Typoraimage-20220211222517727.png)
...
3. 在`Dashboard`中添加`Boom Theme`
   ...
   ![image.png](e863c9e3bcd7a4801f8aa1e9ad37d8697353b603.png@942w_278h_progressive.webp)

   ![image-20220601212433764](image-20220601212433764.png)
...
![Typoraimage-20220217192410413](Typoraimage-20220217192410413.png)
...
![Typoraimage-20220211232919468](Typoraimage-20220211232919468.png)
...
### 卸载 `grafana`
 ...
 1. 查找到安装软件名
    ...
    ![Typoraimage-20220216154734448](Typoraimage-20220216154734448.png)
...
### 配置环境变量 编辑`/etc/profile`并且追加Nginx的环境变量
...
3. 生成 ca 、生成 证书
   ...
   ![Typoraimage-20220216111441217](Typoraimage-20220216111441217.png)

   ![Typoraimage-20220216111555291](Typoraimage-20220216111555291.png)

    ![image-20220602100009735](image-20220602100009735.png)
...
2. 修改 **<font color='red'>netWork.host</font>**, **<font color='red'>http.port</font>** 字段
   ...
   ![Typoraimage-20220215182019551](Typoraimage-20220215182019551.png)

3. 因为是内网测试暂时关闭 xpack 安全验证方面选项,以后需要再去开启

   ![image-20220602101815651](image-20220602101815651.png)
...
![Typoraimage-20220217213204621](Typoraimage-20220217213204621.png)
...
2. 查看 Elasticsearch 端口
   ...
   ![Typoraimage-20220215213238459](Typoraimage-20220215213238459.png)
...
4. 用journal 查看系统日志
   ...
   ![Typoraimage-20220215131826206](Typoraimage-20220215131826206.png)

5. 用 journal 查看elasticsearch 服务日志

   ```sh
   sudo journalctl --unit elasticsearch
   ```

   ![Typoraimage-20220215132103390](Typoraimage-20220215132103390.png)

6. 用journal 查看elasticsearch 指定时间范围的日志

   ```sh
   sudo journalctl --unit elasticsearch --since  "2022-02-01 18:17:16"
   ```

   ![Typoraimage-20220215132318818](Typoraimage-20220215132318818.png)
...
####  启动Lostash	
   ...
   1. 修改 logstash.yml 配置
      ...
      ![Typoraimage-20220218001656526](Typoraimage-20220218001656526.png)
...
​		  收集流程: **<font color='red'>movies.csv->logstash->elasticdearch->grafana</font>**
 ...
 6. 报错
    ...
    2. 执行命令**<font color='red'>sudo /usr/share/logstash/bin/logstash -f /etc/logstash/logstash.conf</font>** 后如果报错
       ...
       ![Typoraimage-20220217124057908](Typoraimage-20220217124057908.png)

       ![Typoraimage-20220217124136069](Typoraimage-20220217124136069.png)

#### 强制查看输出  **<font color='red'>logstash.conf</font>** 修改成你自己的文件
...
#### 查看数据
 ...
 1. 用Kibana的命令行工具执行 **<font color='red'>GET _cat/indices</font>** 命令，就能看见导入到Elasticsearch的索引
    ...
    ![Typoraimage-20220217112819405](Typoraimage-20220217112819405.png)

 2. 用kibana的命令行工具执行**<font color='red'>GET /lua_cpu_monitor-2022.06.03/_search</font>**命令,就能看见导入到Elasticsearch的数据

    ![image-20220603204457448](image-20220603204457448.png)
...
#### 启动Kibana
 ...
 1. 打开kibana.yml 文档
    ...
    ![Typoraimage-20220216215646322](Typoraimage-20220216215646322.png)
...
#### 用filebeat 监控 nginx
 ...
 3. 对比修改下图对应的3个红框地方
    ...
    ![Typoraimage-20220218160818419](Typoraimage-20220218160818419.png)
 ...
 5. 用 **<font color='red'>http:192.168.70.132:7000</font>** 登录nginx 网站生成登录日志，然后打开 **<font color='red'>access.log</font>** 日志
    ...
    ![Typoraimage-20220218161633527](Typoraimage-20220218161633527.png)
